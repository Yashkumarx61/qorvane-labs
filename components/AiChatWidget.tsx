...
* 🧮 **Custom Estimates & Pricing**`,
  ...
  // Helper renderer for basic markdown formatting (bold, links, lists)
  const renderFormattedContent = (content: string) => {
    ...
    return lines.map((line, idx) => {
      ...
      if (line.startsWith("### ")) {
        return (
          <h4 key={idx} className="font-bold text-xs uppercase tracking-wider text-brand-600 dark:text-brand-400 mt-1 mb-1">
            {line.replace("### ", "")}
          </h4>
        );
      }
          ...
          if (match) {
            ...
            return isExternal ? (
              <a
                ...
                className="text-brand-600 dark:text-brand-400 underline font-semibold hover:text-brand-500 inline-flex items-center gap-0.5"
              >
                <span>{label}</span>
              ...
              <Link
                ...
                className="text-brand-600 dark:text-brand-400 underline font-semibold hover:text-brand-500"
              >
                {label}
              </Link>
            );
      ...
      <div className="fixed bottom-6 right-6 z-50 flex items-center gap-3">
        ...
        {!isOpen && (
          <motion.div
            ...
            className="hidden sm:flex items-center space-x-2 px-3.5 py-2 rounded-xl bg-slate-900/90 dark:bg-slate-800/90 text-white text-xs font-semibold shadow-xl backdrop-blur-md cursor-pointer border border-slate-700/80 hover:border-brand-500/50 transition-all"
          >
        ...
        >
          {isOpen ? (
            ...
            <>
              ...
              {hasUnread && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-emerald-500 border-2 border-white dark:border-slate-900 rounded-full animate-pulse" />
              )}
            </>
          )}
        </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-24 right-4 sm:right-6 z-50 w-[92vw] sm:w-[400px] h-[520px] max-h-[78vh] rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 bg-white/95 dark:bg-slate-900/95 text-slate-900 dark:text-slate-100 backdrop-blur-xl flex flex-col overflow-hidden"
          >
            {/* Modal Header */}
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/90 dark:bg-slate-800/60 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="flex items-center space-x-1.5">
                    ...
                    <span className="px-1.5 py-0.2 rounded bg-brand-500/10 text-brand-600 dark:text-brand-400 text-[10px] font-extrabold uppercase">
                      AI
                    </span>
                  </div>
            ...
            <div className="flex-1 p-4 space-y-3.5 overflow-y-auto custom-scrollbar text-xs">
                    ...
                    <div
                      className={`p-3 rounded-2xl max-w-[88%] leading-relaxed ${
                        isUser
                          ...
                          : "bg-slate-100 dark:bg-slate-800/80 text-slate-800 dark:text-slate-200 rounded-bl-none border border-slate-200 dark:border-slate-700/60 shadow-sm"
                      }`}
                    >
              ...
              {/* Typing indicator */}
              {isLoading && (
                <div className="flex items-center space-x-2 p-3 rounded-2xl rounded-bl-none bg-slate-100 dark:bg-slate-800/80 max-w-[100px] border border-slate-200 dark:border-slate-700/60">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {quickPrompts.map((prompt, idx) => (
                    <button
                      ...
                      className="px-2.5 py-1 rounded-full border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-[11px] text-slate-700 dark:text-slate-300 hover:border-brand-500 hover:text-brand-600 dark:hover:text-brand-400 transition-all text-left truncate max-w-full"
                    >
                      {prompt}
                    </button>
                  ))}
            ...
            >
              <input
                ...
                disabled={isLoading}
                className="flex-1 px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 text-xs text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-500 disabled:opacity-50"
              />
              <button
                type="submit"
