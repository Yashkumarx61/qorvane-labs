        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {filteredData.map((cs) => (
            ...
            >
              ...
              {/* Action Buttons */}
              <div className="p-6 pt-0 border-t border-slate-100 dark:border-slate-800/80 mt-4 space-y-2">
                {cs.liveUrl && (
                  <a
                    href={cs.liveUrl}
                    target="_blank"
                ...
                <button
                  ...
                  className="w-full py-2 px-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-800 dark:text-slate-200 font-semibold text-xs flex items-center justify-center space-x-1.5 transition-colors"
                >
        ...
        {selectedCase && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
            ...
            <motion.div
              ...
              className="relative w-full max-w-3xl max-h-[85vh] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-y-auto z-10 p-6 sm:p-10 space-y-6"
            >
              <button
                onClick={() => setSelectedCase(null)}
              ...
              <div className="space-y-2">
                <span className="px-3 py-1 rounded-full bg-brand-500/10 text-brand-600 dark:text-brand-400 text-xs font-bold uppercase">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-xs space-y-1">
                  <h4 className="font-bold text-amber-600 dark:text-amber-400 uppercase">The Challenge:</h4>
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedCase.challenge}</p>
                </div>
                <div className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-xs space-y-1">
                  <h4 className="font-bold text-emerald-600 dark:text-emerald-400 uppercase">The Solution:</h4>
                  <p className="text-slate-700 dark:text-slate-300 leading-relaxed">{selectedCase.solution}</p>
                </div>
              </div>
              ...
              <div className="pt-4 flex items-center justify-between">
                ...
                <button
                  onClick={() => setSelectedCase(null)}
                  className="px-6 py-2.5 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900 text-xs font-bold"
                >
                  Close Specification
                </button>
              </div>
